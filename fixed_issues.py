...
class Comparisons:
    ...
    def extracting_project_assignee_issue_date(self):
        ...
        for pkl_file in pickle_files:
            ...
            issue_status_history = defaultdict(
                            ...
                            for assignee in self.issue_assignee[issue]:
                                ...
                                for project, assignees in project_assignee_map.items():
                                    if str(assignee) in assignees:
                                        issue_status_history[project][issue].append((date, status, actual))
            ...
            # Check for reopened issues per project
            for project, issues in issue_status_history.items():
                ...
                project_reopened[project] = reopened_percentage

                # Print result
                print(
                    f"Pickle: {pkl_file} | Project: {project} | {reopened_issues}/{total_issues} * 100 = {reopened_percentage:.2f}%")

            # Store this pickle file's results in a separate dictionary
            with open(f"{pkl_file}_fixed_issues.pkl", 'wb') as f:
                pkl.dump(project_reopened, f)
            ...
            # Store this pickle file's results in a separate dictionary
            with open(f"{pkl_file}_fixed_issues_minor.pkl", 'wb') as f:
            ...
            # Store this pickle file's results in a separate dictionary
            with open(f"{pkl_file}_fixed_issues_major.pkl", 'wb') as f:
            ...
            # Store this pickle file's results in a separate dictionary
            with open(f"{pkl_file}_fixed_issues_critical.pkl", 'wb') as f:
            ...
            # Store this pickle file's results in a separate dictionary
            with open(f"{pkl_file}_fixed_issues_trivial.pkl", 'wb') as f:
            ...
            # Store this pickle file's results in a separate dictionary
            with open(f"{pkl_file}_fixed_issues_blocker.pkl", 'wb') as f:
